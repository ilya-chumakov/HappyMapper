...
namespace OrdinaryMapper
{
    ...
    public class TypeDetails
    {
        public TypeDetails(Type type, MapperConfigurationExpression config)
        {
            ...
            PublicNoArgExtensionMethods = BuildPublicNoArgExtensionMethods(new MethodInfo[] {});
            ...
            DestinationMemberNames = AllMembers.Select(
                mi => new DestinationMemberName
                {
                    //Member = mi, Possibles = PossibleNames(mi.Name, config.Prefixes, config.Postfixes).ToArray()
                    Member = mi, Possibles = PossibleNames(mi.Name, new string[] { }, new string[] { }).ToArray()
                });
        }
